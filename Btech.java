...
public class Btech
{
void addBook(Statement st,Lib obj) throws IOException,SQLException
{
	...
	String insert="insert into Btech values('"+obj.bookname+"','"+obj.bookauthor+"','"+obj.subject+"','"+obj.stream+"',"+a+","+b+");";
...
void fine(Statement st,issuedBook tmp,Connection con) throws SQLException,IOException,ParseException
{
	...
	ResultSet rs=st.executeQuery("select * from issuedBtech");
	...
	cal2.set((int)Integer.parseInt(s[0]),(int)Integer.parseInt(s[1]),(int)Integer.parseInt(s[2]));
		...
		for(issuedBook pp:al)
		{
			...
			String insert="UPDATE issuedBtech set fine=? where RollNo=? and BookName=?";
...
void issueBook(Statement st,issuedBook obj) throws IOException,SQLException,ParseException
{
	...
	if(cmp==1)
	{
	String insert="insert into issuedbtech values("+a+",'"+obj.bookname+"','"+obj.date+"',0);";
	int val=st.executeUpdate(insert);
	}
	else{
...
void dispAll(Statement st,Lib tmp) throws SQLException
{
	ArrayList <Lib>al=new ArrayList<Lib>();
	...
	while(rs.next()){
		...
		tmp.stream=rs.getString("stream");
		...
		for(Lib pp:al)
		{
			System.out.println(pp.bookname+"\t"+pp.bookauthor+"\t"+pp.subject+"\t"+pp.stream+"\t"+pp.sem+"\t"+pp.year);
		}
}
int compare(Statement st,String bookname) throws SQLException
{
	String search =("SELECT * FROM btech where bookname='"+bookname+"';");
